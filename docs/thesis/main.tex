...
discussed failure mode of DP-SGD on realistic clinical data.
...
\begin{itemize}[leftmargin=*]
  ...
  (\S\ref{sec:future}).
  \item \textbf{We do not use MIT-BIH records~200--234} (which contain more
  arrhythmia variety) despite their being more useful for the rare-class
  classifier we end up unable to train. Records 100--115 are what
  \texttt{wfdb.dl\_database} pulls down by default; we keep them and discuss the
  consequence honestly (Chapter~\ref{ch:dp}, \S\ref{sec:f6.1}).
\end{itemize}
...
\begin{enumerate}[leftmargin=*]
  ...
  more arrhythmias and reduce the imbalance. The framework can run on
  them by changing one config line; we did not because the imbalanced
  case is the more honest workload for an IoT deployment, where the
  monitored patient is overwhelmingly likely to be in normal rhythm.
\end{enumerate}
...
\texttt{wfdb.dl\_database} pulls records 100--115 by default; downloading
the full 48-record set requires authentication and roughly 20\,minutes
on our connection. Because the dominant phenomenon (DP class-prior
collapse) is driven by the workload imbalance, and the imbalance is
preserved as more records are added (records 100--234 are dominated by
N similarly), the qualitative finding is robust to this choice.
Records 200--234 specifically would lower the imbalance and is the
natural follow-up.
...
\begin{enumerate}[leftmargin=*]
  ...
  \item \textbf{Records 200--234} for arrhythmia variety, plus a class-weighted
  loss variant of DP-SGD to test the proposed mitigation in
  Chapter~\ref{ch:dp}, \S5.
